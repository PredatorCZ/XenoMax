...
const TCHAR _info[] = _T(CATTEDTEXT(\nCopyright(C) 2017-2019 Lukas Cone\nVersion\x20 , XENOMAX_VERSION));
const TCHAR _license[] = _T("Xenoblade Tool uses XenoLib, Copyright(C) 2017-2019 Lukas Cone.");
const TCHAR _homePage[] = _T("https://lukascone.wordpress.com/2019/06/04/xenoblade-chronicles-3ds-max-plugin/");

#include "MAXex/win/AboutDlg.h"

XenoImport::XenoImport(): CFGFile(nullptr), hWnd(nullptr),
IDConfigValue(IDC_EDIT_SCALE)(145.f), flags(IDC_CH_DEBUGNAME_checked)
{
	LoadCFG();
}
...
static INT_PTR CALLBACK DialogCallbacks(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
	...
	case WM_INITDIALOG:
	{
		...
		SetWindowText(hWnd, _T(CATTEDTEXT(Xenoblade Import v, XENOMAX_VERSION)));
		...
		for (auto &p : imp->motions)
			SendMessage(imp->comboHandle, CB_ADDSTRING, 0, (LPARAM)esString(p.name).ToTString<TCHAR>().c_str());

		SendMessage(imp->comboHandle, CB_SETCURSEL, imp->IDC_CB_MOTIONINDEX_index >= imp->motions.size() ? 0 : imp->IDC_CB_MOTIONINDEX_index, 0);
	...
	case WM_SIZE:
	{
		...
		SetWindowPos(butt, 0, rekt.right - imp->button1Distance, buttRect.top, 0, 0, SWP_NOZORDER | SWP_NOSIZE | SWP_NOOWNERZORDER);

		butt = GetDlgItem(hWnd, IDC_BT_CANCEL);
		SetWindowPos(butt, 0, rekt.right - imp->button2Distance, buttRect.top, 0, 0, SWP_NOZORDER | SWP_NOSIZE | SWP_NOOWNERZORDER);

		butt = GetDlgItem(hWnd, IDC_CB_MOTIONINDEX);
		GetClientRect(butt, &buttRect);

		SetWindowPos(butt, 0, 0, 0, rekt.right - rekt.left - 15, buttRect.bottom, SWP_NOZORDER | SWP_NOMOVE | SWP_NOOWNERZORDER);
	...
	case WM_COMMAND:
		...
		MSGCheckbox(IDC_CH_BC5BCHAN); break;
