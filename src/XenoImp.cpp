...
public:
	...
	int	LoadMXMD(const TCHAR *name, ImpInterface *importerInt, Interface *ip, BOOL suppressPrompts);
};
...
const TCHAR *XenoImp::LongDesc()
{
	return _T("Xenoblade Inport");
}
	
const TCHAR *XenoImp::ShortDesc()
{			
	return _T("Xenoblade Import");
}

const TCHAR *XenoImp::AuthorName()
{			
	return _T("Lukas Cone");
}

const TCHAR *XenoImp::CopyrightMessage()
{	
	return _T("Copyright (C) 2017-2019 Lukas Cone");
}

const TCHAR *XenoImp::OtherMessage1()
{		
	return _T("");
}

const TCHAR *XenoImp::OtherMessage2()
{		
	return _T("");
}

unsigned int XenoImp::Version()
{				
	return XENOMAX_VERSIONINT;
}

void XenoImp::ShowAbout(HWND hWnd)
{			
	ShowAboutDLG(hWnd);
}
...
void XenoImp::LoadSkeleton(BCSKEL *skel)
{
	...
	for (int b = 0; b < boneData->boneLinks.count; b++)
	{
		...
		Matrix3 nodeTM = {};
		nodeTM.SetRotate(reinterpret_cast<const Quat &>(boneTM.rotation).Conjugate());
		nodeTM.SetTrans(reinterpret_cast<const Point3 &>(boneTM.position) * IDC_EDIT_SCALE_value);
...
void XenoImp::LoadAnimation(BCANIM *anim)
{
	...
	for (int a = 0; a < numAniBones; a++)
	{
		...
		for (auto &t : frameTimes)
		{
			...
			cMat.SetTrans(reinterpret_cast<Point3 &>(evalTransform.position) * IDC_EDIT_SCALE_value);
...
void XenoImp::LoadModelPose(MXMDModel::Ptr &model)
{
	...
	for (int b = 0; b < numBones; b++)
	{
		...
		if (!node)
		{
			...
			nodeTM.SetRow(3, reinterpret_cast<const Point3 &>(bneMat->m[3]) * IDC_EDIT_SCALE_value);
			nodeTM.Invert();
...
void XenoImp::ApplySkin(MXMDGeomBuffers::Ptr &buff, MXMDMeshObject::Ptr &mesh, INodeSuffixer &nde, Face *mfac, MXMDVertexDescriptor *skinDesc)
{
	if (!remapNodes.size())
		return;
	...
	const int skindesc = ((mesh->GetLODID() << 8) & 0xff00) | mesh->GetSkinDesc() & 0xff;
	...
	ISkinImportData *cskin = (ISkinImportData *)cmod->GetInterface(I_SKINIMPORTDATA);
...
void XenoImp::ApplyMorph(MXMDMorphTargets::Ptr &morphs, INode *node, Mesh *mesh, MXMDMeshObject::Ptr &msh, MXMDModel::Ptr &mdl, MXMDGeomBuffers::Ptr &buff)
{
...
int XenoImp::LoadARC(const TCHAR *filename, BOOL suppressPrompts, bool subLoad)
{
	SAR arcFile;
...
int XenoImp::LoadANM(const TCHAR *filename, BOOL suppressPrompts, bool subLoad)
{
	BC anmFile;
...
int XenoImp::LoadMOT(const TCHAR *filename, BOOL suppressPrompts, bool subLoad)
{
	SAR arcFile;
	int loadResult = arcFile.Load(filename, !subLoad);
...
int XenoImp::LoadMXMD(const TCHAR *filename, ImpInterface *importerInt, Interface *ip, BOOL suppressPrompts)
{
	...
	{
		TSTRING hkcfgpath = IPathConfigMgr::GetPathConfigMgr()->GetDir(APP_PLUGCFG_DIR);
		hkcfgpath.append(_T("\\HavokImpSettings.ini"));
		WriteText(_T("HK_PRESETS_HEADER"), _T("Xenoblade"), hkcfgpath.c_str(), _T("currentPresetName"));

		SceneImport *hkImportInterface = static_cast<SceneImport *>(CreateInstance(SCENE_IMPORT_CLASS_ID, HavokImport_CLASS_ID));
...
int XenoImp::DoImport(const TCHAR* filename, ImpInterface* importerInt, Interface* ip, BOOL suppressPrompts)
{
